    ...
    try {
      ...
      if (type === 'Customers List') {
        ...
        const rows = list.map(l => [
          ...
          l.items && l.items.length > 0 ? l.items.map(i => i.itemName).join(" + ") : (l.itemId?.itemName || "Unknown"),
          l.items && l.items.length > 0 ? Array.from(new Set(l.items.map(i => i.category))).join(", ") : (l.itemId?.category || "Unknown"),
          l.items && l.items.length > 0 ? l.items.reduce((sum, i) => sum + (Number(i.netWeight) || 0), 0) : (l.itemId?.netWeight || 0),
          l.items && l.items.length > 0 ? l.items.reduce((sum, i) => sum + (Number(i.grossWeight) || 0), 0) : (l.itemId?.grossWeight || 0),
          l.items && l.items.length > 0 ? l.items.reduce((sum, i) => sum + (Number(i.estimatedValue) || 0), 0) : (l.itemId?.estimatedValue || 0),
          l.loanAmount,
